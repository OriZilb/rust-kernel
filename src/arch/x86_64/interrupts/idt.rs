...
as defined in the kernel/interrupts/consts file.
*/
use crate::arch::x86_64::interrupts::initializations::PICS;
use crate::arch::x86_64::interrupts::initializations::PIC_1_OFFSET;
use crate::interrupts::consts::HANDLERS_PER_IRQ_NUMBER;
use crate::interrupts::irq_dispatch_table::IrqDispatchTable;
...
pub(super) const TOTAL_IRQS_NUMBER: usize = 16;
...
lazy_static! {
    ...
    pub static ref DISPATCH_TABLE: Mutex<IrqDispatchTable<HANDLERS_PER_IRQ_NUMBER, TOTAL_IRQS_NUMBER>> = Mutex::new(IrqDispatchTable::new());
}
...
pub(super) fn common_irq_handler(stack_frame: InterruptStackFrame, irq_num: usize) {
    DISPATCH_TABLE.lock().run_handler(irq_num - PIC_1_OFFSET as usize, &stack_frame);
